...
```
*(Trong đó `details` có thể chứa mảng các lỗi validation nếu có).*
...
```
*(Bạn có thể viết một custom exception handler trong FastAPI để map cấu trúc mặc định này về cấu trúc Standard Error Format ở trên).*
